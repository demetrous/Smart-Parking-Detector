...
class OccupancyDetector:
    ...
    def __init__(
        ...
        """
        Args:
            slots: Slot configs from the camera config file.
            model_name: Ultralytics model name or path. "yolo11n.pt" is the
                        nano variant (fast, small). Use "yolo11s.pt" or larger
                        for better accuracy.
            iou_threshold: Minimum IoU between a vehicle bbox and a slot bbox
                           to consider the slot occupied.
            debounce_frames: Consecutive frames required before a status change
                             is emitted.
            on_update: Callback(slot_id, new_status, confidence) fired when a
                       slot's confirmed status changes.
        """
    ...
    def process_frame(self, frame: np.ndarray) -> None:
        """Run detection on one frame and update slot states."""
    ...
    def annotate_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        for state in self._states.values():
            ...
            color = (0, 200, 0) if state.current_status == "available" else (0, 0, 220)
