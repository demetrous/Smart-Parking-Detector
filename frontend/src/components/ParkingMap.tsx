...
export default function ParkingMap({ className, spotsOverride, hideRecentlyOccupied }: ParkingMapProps) {
  const { theme } = useTheme();
  const token = import.meta.env.VITE_MAPTILER_KEY as string | undefined;

  if (!isUsableMaptilerKey(token)) {
    return (
      <div className={clsx('grid h-dvh w-full place-items-center px-6 text-center', className)}>
  ...
  return (
    <div className={clsx('h-dvh w-full', className)}>
      <Map
        reuseMaps
      ...
      >
        <MapMarkers spotsOverride={spotsOverride} hideRecentlyOccupied={hideRecentlyOccupied} />
      </Map>
    </div>
  );
}
